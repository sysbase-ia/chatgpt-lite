...
function deriveSessionCloseURL(): string {
  const explicit = process.env.CONTROL_PLANE_TOPOLOGY_URL?.trim()
  if (explicit) {
    try {
      const u = new URL(explicit)
      u.pathname = '/topology/sessions/close'
      u.search = ''
      u.hash = ''
      return u.toString()
    } catch {
  ...
  return 'http://127.0.0.1:8080/topology/sessions/close'
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const response = await fetch(deriveSessionCloseURL(), {
      method: 'POST',
      cache: 'no-store',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          error: parsed.error || `control-plane session close http ${response.status}`,
